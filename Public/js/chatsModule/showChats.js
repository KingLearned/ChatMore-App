const showChats = (ChatLogs, FRD, SORT, userName, MainChats) => {
    for (let i = 0; i < FRD.length; i++) {
        $(`.chat_${FRD[i]}`).on('click', () => {
            ...
            `<user_${FRD[i]} style='position: absolute; bottom: 0; right: 0; width: 13px; height: 13px; background-color: ${setStatus == 'online' ? 'lime' : 'red'}; border-radius: 100px;'></user_${FRD[i]}>`
